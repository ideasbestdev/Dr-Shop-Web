import { LoaderStyle, PageNumberStyle, ProductListSectionStyle, SectionTitleStyle, SelectStyle } from '@/styledcomponents/index'
...
export function ProductListSection() {
    ...
    return (
        <>
            <ProductListSectionStyle className={pageData.productList && pageData.productList.length > 0 ? "" : "noItems"}>
                <div>
                    <SelectStyle>
                ...
                {
                    pageData.productList && pageData.productList.length > 0 ? <ul>
                        {
                            pageData.productList.map((value: ProductModel, index: number) => <li key={value.id}><ProductItemComponent product={value} /></li>)
                        }

                    </ul> : !pageData.loading ? <SectionTitleStyle>No products found</SectionTitleStyle> : null
